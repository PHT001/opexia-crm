    ...
    <div className="p-4 lg:p-6 space-y-6 pt-16 lg:pt-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#636366] mt-1">{charges.length} charge{charges.length > 1 ? 's' : ''} au total · {activeCharges.length} active{activeCharges.length > 1 ? 's' : ''}</p>
        </div>
        <button
          onClick={() => { setEditingCharge(null); setModalOpen(true); }}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#0a84ff] hover:bg-[#409cff] text-white text-sm font-medium transition-colors btn-glow"
        >
          <Plus size={16} />
          Nouvelle charge
        </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="kpi-glow rounded-2xl p-4 shine-top">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-[#8e8e93] font-medium uppercase tracking-wide">Total /mois</span>
            <div className="p-2 rounded-xl icon-glow-warning">
          ...
          <p className="text-xs text-[#636366] mt-1">{activeCharges.length} charge{activeCharges.length > 1 ? 's' : ''} active{activeCharges.length > 1 ? 's' : ''}</p>
        </div>
        <div className="kpi-glow rounded-2xl p-4 shine-top">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-[#8e8e93] font-medium uppercase tracking-wide">Total /an</span>
            <div className="p-2 rounded-xl icon-glow-danger">
          ...
          <p className="text-xs text-[#636366] mt-1">Projection annuelle</p>
        </div>
        <div className="kpi-glow rounded-2xl p-4 shine-top">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-[#8e8e93] font-medium uppercase tracking-wide">CA /mois</span>
            <div className="p-2 rounded-xl icon-glow-primary">
              <BarChart3 size={18} className="text-[#0a84ff]" />
            </div>
          </div>
          <p className="text-2xl font-bold text-foreground">{caMensuel.toLocaleString('fr-FR')} €</p>
          <p className="text-xs text-[#636366] mt-1">Revenus récurrents</p>
        </div>
        <div className="kpi-glow rounded-2xl p-4 shine-top">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-[#8e8e93] font-medium uppercase tracking-wide">Bénéfice /mois</span>
            <div className={`p-2 rounded-xl ${beneficeMensuel >= 0 ? 'icon-glow-success' : 'icon-glow-danger'}`}>
              <CreditCard size={18} className={beneficeMensuel >= 0 ? 'text-[#30d158]' : 'text-[#ff453a]'} />
            </div>
          </div>
          <p className={`text-2xl font-bold ${beneficeMensuel >= 0 ? 'text-[#30d158]' : 'text-[#ff453a]'}`}>
            {beneficeMensuel >= 0 ? '+' : ''}{Math.round(beneficeMensuel).toLocaleString('fr-FR')} €
          </p>
          <p className="text-xs text-[#636366] mt-1">{beneficeMensuel >= 0 ? 'Rentable' : 'Déficitaire'}</p>
        </div>
      </div>
                ...
                {chargesByCategory.map((item) => (
                  <div key={item.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs text-[#8e8e93]">{item.name}</span>
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-semibold text-foreground tabular-nums">{item.value} €</span>
                      <span className="text-[10px] text-[#636366] ml-1">({item.count})</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.08] text-foreground text-sm placeholder:text-[#636366] focus:outline-none focus:border-[rgba(10,132,255,0.5)]"
          />
        </div>
        <select
          value={filterCategory}
          onChange={(e) => setFilterCategory(e.target.value as ChargeCategory | 'toutes')}
          className="px-4 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.08] text-foreground text-sm focus:outline-none focus:border-[rgba(10,132,255,0.5)]"
        >
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[#636366] border-b border-white/[0.06] bg-white/[0.02]">
              ...
              {filtered
                ...
                >
                  <td className="px-4 py-3">
                    ...
                    {charge.notes && <p className="text-xs text-[#636366] truncate max-w-[200px]">{charge.notes}</p>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-[#8e8e93] hidden lg:table-cell">{charge.fournisseur || '—'}</td>
                  <td className="px-4 py-3 font-bold text-foreground">{charge.montant.toLocaleString('fr-FR')} €</td>
                  <td className="px-4 py-3 hidden sm:table-cell">
                    <span className="text-xs text-[#8e8e93]">{CHARGE_FREQUENCY_LABELS[charge.frequence]}</span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${
                        charge.actif
                          ? 'bg-[rgba(48,209,88,0.1)] text-[#30d158] hover:bg-[rgba(48,209,88,0.15)]'
                          : 'bg-white/[0.04] text-[#636366] hover:bg-white/[0.06]'
                      }`}
                    >
